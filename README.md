...
**Podman**: `podman compose -f compose.yml up -d`. **LXC / VM**: run the same image via Docker/Podman on the guest, or install upstream. **Unraid**: *Docker → Add Container* with the image/ports/volumes from `compose.yml`. Upstream: <https://www.home-assistant.io/>.
