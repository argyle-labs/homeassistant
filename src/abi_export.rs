...
//! own statically-linked tool inventory (the `home-assistant.*` entity/service
//! tools + the `endpoint_resource!` CRUD) through the toolkit's re-exported
//! dispatch surface.
//!
...
/// tools alongside the plugin's. The plugin exposes only its own
/// `home-assistant.*` namespace across the ABI; the host already owns the domain
/// tools and would otherwise reject the manifest as colliding built-ins.
const TOOL_PREFIX: &str = "home-assistant.";

/// The plugin's own tool surface: `tool_manifest_json()` filtered to the
/// `home-assistant.*` namespace. Shared by `manifest()` (serialized back out)
/// and `invoke()` (admission check) so both agree on exactly which tools cross.
...
/// exposes (HTTP-only endpoint CRUD + entity/service) needs no host-injected
